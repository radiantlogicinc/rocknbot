...
class LiteLLMMistralLLM(LLM):
    ...
    def stream_chat(self, messages: Sequence[ChatMessage], **kwargs: Any) -> Generator[ChatResponse, None, None]:
        ...
        for chunk in response:
            yield ChatResponse(message=ChatMessage(role="assistant", content=chunk["choices"][0]["delta"].get("content", "")))
...
# Utility Functions
# -----------------------------------------------------------------------------
def get_llsc(session_id: str, locale: Optional[LOCALE] = None, product: Optional[PRODUCT] = None) -> LilLisaServerContext:
    """
...
async def record_endorsement(session_id: str, is_expert: bool) -> str:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Internal error in record_endorsement() for session_id: {session_id}") from exc
...
async def update_golden_qa_pairs(product: str, encrypted_key: str) -> str:
    ...
    try:
        ...
        version_pattern = re.compile(r"v?\d+\.\d+", re.IGNORECASE) if product == "IDDM" else re.compile(r"\b(?:IAP[- ]\d+\.\d+|version[- ]\d+\.\d+|descartes(?:-dev)?)\b", re.IGNORECASE)
...
async def get_conversations(product: str, endorsed_by: str, encrypted_key: str) -> StreamingResponse:
    ...
    try:
        jwt.decode(encrypted_key, AUTHENTICATION_KEY, algorithms="HS256")
        session_ids = [entry for entry in os.listdir(LilLisaServerContext.SPEEDICT_FOLDERPATH) 
        ...
        for session_id in session_ids:
            ...
            if product_enum == llsc.product:
                endorsements = llsc.user_endorsements if endorsed_by == "user" else llsc.expert_endorsements if endorsed_by == "expert" else None
                if endorsements:
    ...
    try:
        ...
        def extract_metadata_from_lines(lines):
            ...
            for line in lines:
                ...
                elif line.startswith("description:"): metadata["description"] = line.split(":", 1)[1].strip()
                elif line.startswith("keywords:"): metadata["keywords"] = line.split(":", 1)[1].strip()
            return metadata
        ...
        excluded_metadata_keys = ["file_path", "file_name", "file_type", "file_size", "creation_date", "last_modified_date", "version", "github_url"]
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    for branch in branches:
                        ...
                        for file in md_files:
                            ...
                            for doc in documents:
                                ...
                                github_url = f"https://github.com/radiantlogic-v8/{relative_path}".replace(repo_name, f"{repo_name}/blob")
                ...
                for node in all_nodes:
                    if len(enc.encode(node.text)) > 7000:
                        sub_nodes = splitter.get_nodes_from_documents([Document(text=node.text, metadata=node.metadata)])
